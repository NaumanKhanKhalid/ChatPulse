...
<div class="screen">
  ...
  <form class="mt-8 space-y-5" method="POST" action="{{ route('guest-login') }}">
    ...
    <input type="hidden" name="_loaded_at" id="loadedAt" />
    ...
    <div>
      ...
      <div class="flex items-center gap-2.5 rounded-xl border bg-white px-3.5 h-12 transition focus-within:border-primary focus-within:ring-4 focus-within:ring-primary-light/60 {{ $errors->has('name') ? 'border-busy' : 'border-line' }}">
        <div class="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 text-[11px] font-bold text-white" id="guestInitials">?</div>
      ...
      <p class="mt-1.5 text-[12px] text-ink-400">Minimum 2 characters. You can change it later.</p>
    ...
    <div class="rounded-xl border border-amber-200 bg-amber-50 p-4" style="background-color: rgba(255,251,235,0.7);">
    ...
    <button type="submit" id="guestSubmit" disabled
            class="group flex h-12 w-full items-center justify-center gap-2 rounded-xl text-[15px] font-bold text-white transition active:scale-[.99] disabled:cursor-not-allowed"
            style="background:#8a958f;">
      Start chatting
...
(function() {
  ...
  const initials = document.getElementById('guestInitials');
  const btn = document.getElementById('guestSubmit');

  function update() {
    const n = nameInput.value.trim();
    const parts = n.split(/\s+/);
    const ini = ((parts[0]?.[0] || '') + (parts[1]?.[0] || '')).toUpperCase() || (n[0] || '?').toUpperCase();
    initials.textContent = ini;
    const valid = n.length >= 2;
    btn.disabled = !valid;
    btn.style.background = valid ? '#10b981' : '#8a958f';
    btn.style.boxShadow = valid ? '0 8px 20px -8px rgba(16,185,129,0.7)' : 'none';
  }

  nameInput.addEventListener('input', update);

  // init with old value
  if (nameInput.value) update();
})();
</script>
@endpush
@endsection
